...
from odoo.addons.mozaik_committee.wizards.electoral_results_wizard import (
...
class TestElectoralResultsWizard(TransactionCase):
    ...
    def setUp(self):
        ...
        self.sta_pauline_communal = self.env.ref('mozaik_committee.sta_pauline_communal')
        self.sta_marc_communal = self.env.ref('mozaik_committee.sta_marc_communal')
        self.sta_thierry_communal = self.env.ref('mozaik_committee.sta_thierry_communal')
        self.sta_jacques_communal = self.env.ref('mozaik_committee.sta_jacques_communal')

        self.committee_id = self.env.ref('mozaik_committee.sc_tete_huy_communale')

        accepted_ids = (self.sta_paul_communal
    ...
    def test_electoral_results_wizard_wrong_file(self):
        ...
        data = [self.district.name, '', self.sta_marc_communal.partner_id.name,
                '3', '2', '']
        temp_file.write(','.join(data) + '\n')
        # elected candidate with position non elected set
        data = [self.district.name, '', self.sta_paul_communal.partner_id.name,
                '3', '', '1']
        temp_file.write(','.join(data) + '\n')
        # inconsistent value for column E/S
        data = [self.district.name, 'B',
                self.sta_pauline_communal.partner_id.name, '3', '2', '1']
        temp_file.write(','.join(data) + '\n')
        # inconsistent value for column E/S with candidature settings
        data = [self.district.name, '',
                self.sta_thierry_communal.partner_id.name, '3', '2', '1']
        temp_file.write(','.join(data) + '\n')
        # Effective line with substitute candidature
        data = [self.district.name, 'E',
                self.sta_thierry_communal.partner_id.name, '3', '2', '1']
        temp_file.write(','.join(data) + '\n')
        # Substitute line with effective candidature
        data = [self.district.name, 'S',
                self.sta_pauline_communal.partner_id.name, '3', '2', '1']
        temp_file.write(','.join(data) + '\n')
        ...
        data = [self.district.name, '',
                self.sta_paul_communal.partner_id.name, '3', '2', '1']
        temp_file.write(','.join(data) + '\n')
        temp_file.seek(0)
        ...
        wiz_id = self.env['electoral.results.wizard'].with_context(context).create(
    ...
    def test_electoral_results_wizard_elected(self):
        ...
        data = [self.district.name, '',
                self.sta_paul_communal.partner_id.name, '1258', '1', '']
        temp_file.write(','.join(data) + '\n')

        data = [self.district.name, 'E',
                self.sta_pauline_communal.partner_id.name, '1258', '1', '']
        temp_file.write(','.join(data) + '\n')

        data = [self.district.name, 'S',
                self.sta_jacques_communal.partner_id.name, '1258', '1', '']
        temp_file.write(','.join(data) + '\n')
        data = [self.district.name, 'E',
                self.sta_jacques_communal.partner_id.name, '1258', '1', '']
        temp_file.write(','.join(data) + '\n')
    ...
    def test_electoral_results_wizard_non_elected(self):
        ...
        data = [self.district.name, 'E',
                self.sta_pauline_communal.partner_id.name, '1258', '0', '']
        temp_file.write(','.join(data) + '\n')
        ...
        temp_file.write(','.join(data) + '\n')
        data = [self.district.name, 'E',
                self.sta_jacques_communal.partner_id.name, '1258', '0', '']
        temp_file.write(','.join(data) + '\n')
