...
class PartnerInvolvementFollowupWizard(models.TransientModel):
    ...
    def _next_category_ids_domain(self):
        ...
        if inv_ids:
            current_fols = (
                self.env["partner.involvement"]
                .browse(inv_ids)
                .mapped("involvement_category_id")
            )
            next_fols = (
                current_fols
                and current_fols[0].involvement_category_ids
                or self.env["partner.involvement.category"]
            )
    ...
    def _get_followup(self):
        types = [
            (
                "delay",
                _(
                    "I added some other followers, follow-up continue, "
            ...
            (
                "done",
                _(
                    "Follow-up is done, I added details in "
