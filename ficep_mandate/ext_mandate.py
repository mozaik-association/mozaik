...
# view methods: onchange, button
    ...
    def process_invalidate_candidatures_after_delay(self, cr, uid, context=None):
        ...
        :rtype: boolean
        """
        return super(ext_selection_committee, self).process_invalidate_candidatures_after_delay(cr, uid, context=context)
...
class ext_candidature(orm.Model):
    ...
    _mandate_category_store_trigger = {
        ...
        _selection_committee_model: (lambda self, cr, uid, ids, context=None:
                                    self.pool.get('ext.candidature').search(cr, uid, [('selection_committee_id', 'in', ids)], context=context),
    ...
    _columns = {
        ...
        'mandate_category_id': fields.related('selection_committee_id', 'mandate_category_id', string='Mandate Category',
                                          ...
                                          store=_mandate_category_store_trigger, domain=[('type', '=', 'ext')]),
    ...
    _order = 'ext_assembly_id, mandate_start_date, mandate_category_id, partner_name'
...
class ext_mandate(orm.Model):
    ...
    _unique_id_store_trigger = {
            'ext.mandate': (lambda self, cr, uid, ids, context=None: ids, ['partner_id'], 20),
    }

    def _compute_unique_id(self, cr, uid, ids, fname, arg, context=None):
        return super(ext_mandate, self)._compute_unique_id(cr, uid, ids, fname, arg, context=context)
