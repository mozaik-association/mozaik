                ...
                SELECT *
                  FROM (
                    SELECT 'int.mandate' AS model,
                           ...
                           CASE WHEN mandate.months_before_end_of_mandate > 0 THEN mandate.months_before_end_of_mandate
                                WHEN assembly.months_before_end_of_mandate > 0 THEN assembly.months_before_end_of_mandate
...
# public methods
    ...
    def process_notify_ending_mandates(self, cr, uid, ids=None, context=None):
        ...
        for mandate_model in self._mandate_models:
            mandates = self.search_read(cr, uid, domain=[('model', '=', mandate_model)], context=context)
            for mandate in mandates:
                secretariat_id = assembly_pool.get_secretariat_assembly_id(cr, uid, mandate['designation_int_assembly_id'], context=context)
                if secretariat_id:
                    mandate = self.pool.get(mandate['model']).browse(cr, uid, mandate['id'], context=context)
        ...
        for secretary_id in alert_dict:
            secretary = assembly_pool.browse(cr, uid, secretary_id, context=context)
            mandate_list = alert_dict[secretary_id]
            content_text = ['<p><table>']
            content_text.append('<tr><th colspan="2"><br/><u>%s</u></th></tr>' % _('Ending mandates alerts report'))
            for mandate in mandate_list:
                ...
                content_text.append('<tr><td><b>%s</b</td><td>%s</td><td>%s</td><td><a style="color:blue" href="%s">(%s)</a></td></tr>' % \
            ...
            mail_vals = {
                'subject': _('Ficep: Ending Mandates Summary - %s') % self.pool.get('res.lang').format_date(cr, uid, fields.date.today(cr, uid), context=context),
                'body_html': '\n'.join(content_text),
