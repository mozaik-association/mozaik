...
class import_sta_candidatures_wizard(orm.TransientModel):
    ...
    def validate_file(self, cr, uid, ids, context=None):
        ...
        for line in file_lines:
            ...
            if len(data) != len(file_import_structure):
                raise orm.except_orm(_('Error'), _('Line %s: a wrong number of columns(%s), %s expected !' % (line_number, len(data), len(file_import_structure))))
            ...
            if not wizard.selection_committee_id.assembly_id.is_legislative:
                ...
                if data[file_import_structure.index('is_substitute')] == 'True':
                    raise orm.except_orm(_('Error'), _('Line %s: Substitute feature is not managed for a non-legislative assembly !' % line_number))

            values = dict(wizard_id=wizard.id,
                        partner_id=partner_id,
                        partner_name=data[file_import_structure.index('partner_name')] if data[file_import_structure.index('partner_name')] != '' else False,
                        is_effective=data[file_import_structure.index('is_effective')] if data[file_import_structure.index('is_effective')] != '' else False,
    ...
    def import_candidatures(self, cr, uid, ids, context=None):
        ...
        for line in wizard.import_lines:
            candidature_ids = candidature_pool.search(cr, uid, [('selection_committee_id', '=', wizard.selection_committee_id.id),
            ...
            candidature_values = dict(partner_id=line.partner_id.id,
                        ...
                        list_effective_position=line.list_effective_position if line.is_effective else False,
                        list_substitute_position=line.list_substitute_position if line.is_substitute else False)

            if candidature_ids:
                candidature_pool.write(cr, uid, candidature_ids[0], candidature_values, context=context)
            else:
                candidature_values['selection_committee_id'] = wizard.selection_committee_id.id
                candidature_pool.create(cr, uid, candidature_values, context=context)
...
class import_sta_candidature_file_lines (orm.TransientModel):
    ...
    _columns = {
        ...
        'list_effective_position': fields.integer('Position on effectives list'),
        'list_substitute_position': fields.integer('Position on substitutes list'),
        }
