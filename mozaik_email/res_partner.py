...
class res_partner(orm.Model):
    ...
    def _get_main_email_coordinate_ids(self, cr, uid, ids, name, args,
                                       context=None):
        """
        ==============================
        _get_main_email_coordinate_ids
        ==============================
        Reset *_coordinate_id fields with corresponding main email coordinate
        ids
        :param ids: partner ids for which new *_coordinate_id fields have to be
                    recomputed
        :type name: char
    ...
    def _get_main_email(self, cr, uid, ids, name, args, context=None):
        ...
        :type name: char
        :rparam: dictionary for all partner ids with the requested main email
                number
    ...
    _email_store_trigger = {
        'email.coordinate':
            (lambda self, cr, uid, ids, context=None:
                self.pool['email.coordinate'].get_linked_partners(
                    cr, uid, ids, context=context),
             ['partner_id', 'email', 'is_main', 'vip', 'unauthorized', 'active'
              ],
             10),
    }
    ...
    _columns = {
        'email_coordinate_ids':
            fields.one2many(
                'email.coordinate', 'partner_id', 'Email Coordinates',
                domain=[('active', '=', True)],
        ...
        'email_coordinate_id':
            fields.function(
                _get_main_email_coordinate_ids, string='Email',
