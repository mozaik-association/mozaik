...
class TestStructure(TransactionCase):
    ...
    def test_power_level_consistency(self):
        ...
        data = {
            "name": "wrong assembly",
            "assembly_category_id": self.ref(
                "mozaik_structure.int_assembly_category_01"
            ),
            "instance_id": self.ref("mozaik_structure.int_instance_02"),
        }

        # create the assembly: NOK
        self.assertRaises(
            exceptions.ValidationError, self.env["int.assembly"].create, data
        )
        ...
        self.assertRaises(
            ...
            self.browse_ref("mozaik_structure.int_instance_02").write,
            data,
        )
        ...
        data = {
            ...
            "instance_id": self.ref("mozaik_structure.sta_instance_02"),
        }

        # create the assembly: NOK
        self.assertRaises(
            exceptions.ValidationError, self.env["sta.assembly"].create, data
        )
        ...
        self.assertRaises(
            exceptions.ValidationError,
            self.browse_ref("mozaik_structure.sta_instance_01").write,
            data,
        )
        ...
        for i in range(1, 3):
            ...
            ext_cat.power_level_id = self.ref(
                "mozaik_structure.int_power_level_0%s" % i
            )
    ...
    def test_company_and_assembly_flag(self):
        ...
        # create the assembly: NOK
        self.assertRaises(
            exceptions.ValidationError, self.env["ext.assembly"].create, data
        )
        ...
        data = {
            ...
            "ref_partner_id": self.browse_ref(
    ...
    def test_instance_recursion(self):
        ...
        # try a recursion: NOK
        self.assertRaises(
            exceptions.UserError, instance1.write, {"parent_id": instance2.id}
        )
        ...
        self.assertRaises(
            exceptions.ValidationError,
            instance1.write,
            {"secondary_parent_id": instance2.id},
        )
    ...
    def test_onchange_assembly_id(self):
        ...
        data = {
            "sta_instance_id": self.ref("mozaik_structure.sta_instance_01"),
            "assembly_id": self.ref("mozaik_structure.sta_assembly_02"),
        }
    ...
    def test_assembly_name(self):
        ...
        # check for its name
        self.assertEqual("%s (%s)" % (instance.name, cat.name), assembly.name)
        ...
        # check for its name
        self.assertEqual("%s (%s)" % (p.name, cat.name), assembly.name)
