...
class ExtAssembly(models.Model):
    ...
    def _get_names(self, vals=None):
        ...
        n1 = (
            self
            and self.ref_partner_id
            or self.env["res.partner"].browse(vals and vals.get("ref_partner_id"))
        ).name or False
        n2 = (
            self
            and self.assembly_category_id
            or self.env["ext.assembly.category"].browse(
    ...
    @api.model
    @api.onchange("ref_partner_id", "assembly_category_id")
