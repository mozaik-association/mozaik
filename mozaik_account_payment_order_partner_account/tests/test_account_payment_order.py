    ...
    def test_force_partner_account(self):
        ...
        receivable_account = self.env["account.account"].search(
            [
                ...
                ("id", "!=", self.partner.property_account_receivable_id.id),
            ],
            limit=1,
        )
        self.company.debit_order_partner_account_id = receivable_account.id
        ...
        self.assertEqual(receivable_account.id, credit_ml.account_id.id)

    def test_not_force_partner_account(self):
        """
        Default behavior is no account forced on company
        """
        self.debit_order.draft2open()
        self.debit_order.open2generated()
        self.debit_order.generated2uploaded()
        ...
        self.assertEqual(
            self.partner.property_account_receivable_id.id, credit_ml.account_id.id
        )
