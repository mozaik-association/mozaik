...
class DistributionList(models.Model):
    ...
    def copy(self, default=None):
        """
        Default copies of distribution list with mail forwarding fail.
        The reason is that alias_name is mandatory and copied, but must be unique.
        The solution is to copy the distribution list without the mail forwarding feature,
        then reactivating it and computing the new alias name.
        NB: Since the alias name is computed from the distribution list name which must
        be unique, we are sure that the alias name will also be unique.
        """
        self.ensure_one()
        default = default or {}
        is_mail_forwarding = self.mail_forwarding
        if is_mail_forwarding:
            default["mail_forwarding"] = False
            ...
            result.write(
                {
                    ...
                    "alias_name": self._build_alias_name(result.name),
                }
            )
            self._onchange_mail_forwarding()
        return result
