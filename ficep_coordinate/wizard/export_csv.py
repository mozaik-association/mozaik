...
class export_csv(orm.TransientModel):
    ...
    def get_csv_values(self, cr, uid, model, obj, context=None):
        ...
        export_values = OrderedDict([('name', _get_utf8(partner.lastname)),
                                     ('lastname',
                                      None if not partner.usual_lastname else _get_utf8(partner.usual_lastname)),
                                     ('firstname', None if not partner.firstname else _get_utf8(partner.firstname)),
                                     ('usual_firstname',
                                      None if not partner.usual_firstname else _get_utf8(partner.usual_firstname)),
                                     ('identifier', partner.identifier or None),
                                     ('printable_name',
                                      _get_utf8(pc.co_residency_id.line) if (pc and pc.co_residency_id.line) \
                                          else _get_utf8(partner.printable_name)),
                                     ('co_residency',
                                      _get_utf8(pc.co_residency_id.line2) if (pc and pc.co_residency_id.line2) \
                                          else None),
                                     ...
                                     ('street',
                                      None if not pc or not pc.address_id.street else _get_utf8(pc.address_id.street)),
                                     ('street2', None if not pc or not pc.address_id.street2 else _get_utf8(
                                         pc.address_id.street2)),
                                     ('city', None if not pc or pc.address_id.city else _get_utf8(pc.address_id.city)),
                                     ('instance',
                                      partner.int_instance_id and _get_utf8(partner.int_instance_id.name) or None),
                                     ...
                                     ('fix_main',
                                      partner.fix_coordinate_id and partner.fix_coordinate_id.is_main or False),
                                     ('fix_unauthorized',
                                      partner.fix_coordinate_id and partner.fix_coordinate_id.unauthorized or False),
                                     ('fix_vip', partner.fix_coordinate_id and partner.fix_coordinate_id.vip or False),
                                     ('fix', None if not partner.fix_coordinate_id else _get_utf8(
                                         partner.fix_coordinate_id.phone_id.name)),
                                     ('mobile_main',
                                      partner.mobile_coordinate_id and partner.mobile_coordinate_id.is_main or False),
                                     ('mobile_unauthorized',
                                      partner.mobile_coordinate_id and partner.mobile_coordinate_id.unauthorized or False),
                                     ('mobile_vip',
                                      partner.mobile_coordinate_id and partner.mobile_coordinate_id.vip or False),
                                     ('mobile', None if not partner.mobile_coordinate_id else _get_utf8(
                                         partner.mobile_coordinate_id.phone_id.name)),
                                     ('fax_main',
                                      partner.fax_coordinate_id and partner.fax_coordinate_id.is_main or False),
                                     ('fax_unauthorized',
                                      partner.fax_coordinate_id and partner.fax_coordinate_id.unauthorized or False),
                                     ('fax_vip', partner.fax_coordinate_id and partner.fax_coordinate_id.vip or False),
                                     ('fax', None if not partner.fax_coordinate_id else _get_utf8(
                                         partner.fax_coordinate_id.phone_id.name)),
