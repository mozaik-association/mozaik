...
class ChangeMainAddress(models.TransientModel):
    ...
    old_address_id = fields.Many2one("address.address", "Current Main Address")
    ...
    invalidate_co_residency = fields.Boolean(
        "Invalidate Co-Residency", default=True
    )
        ...
        if len(ids) == 1:
            ...
            if res.get("old_address_id", False):
                ...
                if co_res:
                    ...
                    res["invalidate_co_residency"] = res.get(
                        "move_allowed", False
                    )
