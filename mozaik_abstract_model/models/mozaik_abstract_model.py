...
class MozaikAbstractModel(models.AbstractModel):
    ...
    def init(self):
        ...
        index_query = (
            "CREATE UNIQUE INDEX %(index_name)s ON %(table_name)s "
    ...
    def create(self, vals):
        ...
        if res and self._context.get("migration") and vals.get("create_date"):
            if vals.get("write_date"):
                ...
                p = (AsIs(self._table), vals["create_date"], vals["write_date"], res.id)
            else:
    ...
    def _check_invalidate(self):
        ...
        if invalidates:
            ...
            if rels_dict:
                ...
                keep_active_models_list = (
                    keep_active_models.split(",") if keep_active_models else []
                )
                for k, v in rels_dict.items():
                    if v in keep_active_models_list:
                        continue
