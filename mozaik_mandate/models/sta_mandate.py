...
class StaMandate(models.Model):
    ...
    sta_assembly_id = fields.Many2one(
        comodel_name="sta.assembly", string="State Assembly", index=True, required=True
    )
    ...
    is_legislative = fields.Boolean(
        related="sta_assembly_id.is_legislative", store=True
    )
    competencies_m2m_ids = fields.Many2many(
        comodel_name="thesaurus.term", string="Remits"
    )
    ...
    def _check_duplicate_mandates(self):
        ...
        for m in self_sudo:
            ...
            duplicates = allm.filtered(
                ...
                p=m.partner_id,
                a=m.sta_assembly_id,
                c=m.mandate_category_id,
                start=m.start_date,
                end=m.end_date or m.deadline_date: s.partner_id
                == p
                and s.sta_assembly_id == a
                and s.mandate_category_id == c
                and s.start_date <= end
                and (s.end_date or s.deadline_date) >= start
            )
            # fmt: on
            if len(duplicates) > 1:
    ...
    def _check_legislature_date_consistency(self):
        ...
        for mandate in self_sudo:
            if not (
                mandate.legislature_id.start_date
                <= mandate.start_date
    ...
    def _onchange_mandate_category_id(self):
        for mdt in self:
            if (
                not mdt.mandate_category_id
                or mdt.mandate_category_id.sta_assembly_category_id
    ...
    def _onchange_legislature_id(self):
        for sta_mandate in self:
            ...
            sta_mandate.deadline_date = sta_mandate.legislature_id.deadline_date
    ...
    def _onchange_sta_assembly_id(self):
        ...
        if self.sta_assembly_id:
            ...
            if assembly.assembly_category_id.is_legislative:
                designation_int_assemblies = assembly.electoral_district_ids.mapped(
            ...
            if len(designation_int_assemblies) == 1:
                designation_int_assembly_id = designation_int_assemblies[0].id
        else:
