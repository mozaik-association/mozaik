...
class TestCopyMandateWizard(SavepointCase):
    ...
    def test_renew_legislative_state_mandate(self):
        ...
        """
        stam_thierry_communal_2012 = self.env.ref(
            "%s.stam_thierry_communal_2012" % self._module_ns
        )
        ...
        stam_thierry_communal_2012.sta_assembly_id.assembly_category_id.is_legislative = (
            True
        )
    ...
    def test_renew_non_legislative_state_mandate(self):
        ...
        """
        stam_thierry_bourgmestre_2012_id = self.ref(
            "%s.stam_thierry_bourgmestre_2012" % self._module_ns
        )
        ...
        self.assertEqual(
            base_mandate.mandate_category_id, new_mandate.mandate_category_id
        )
        ...
        self.assertEqual(
            base_mandate.with_revenue_declaration, new_mandate.with_revenue_declaration
        )
        self.assertEqual(
            base_mandate.with_assets_declaration, new_mandate.with_assets_declaration
        )
    ...
    def test_create_complementary_internal_mandate(self):
        ...
        """
        mandate_object = self.env["int.mandate"]
        stam_paul_regional = self.env.ref("%s.intm_paul_regional" % self._module_ns)
        mc_secretaire_regional_id = self.ref(
            "%s.mc_secretaire_regional" % self._module_ns
        )
    ...
    def test_renew_external_mandate(self):
        ...
        values = dict(start_date="2014-12-03", deadline_date="2020-12-03")
    ...
    def test_create_complementary_external_mandate(self):
        ...
        """
        mandate_object = self.env["ext.mandate"]
        extm_paul_membre_ag = self.env.ref("%s.extm_paul_membre_ag" % self._module_ns)
        ...
        values = dict(
            ...
            new_assembly_id=assembly_id.copy({"assembly_category_id": ac.id}).id,
        )
        wizard.write(values)
        ...
        self.assertEqual(extm_paul_membre_ag.partner_id, new_mandate.partner_id)
        self.assertNotEqual(
            extm_paul_membre_ag.mandate_category_id, new_mandate.mandate_category_id
        )
        self.assertNotEqual(
            extm_paul_membre_ag.ext_assembly_id, new_mandate.ext_assembly_id
        )
        self.assertEqual(
            extm_paul_membre_ag.with_revenue_declaration,
            new_mandate.with_revenue_declaration,
        )
        self.assertEqual(
            extm_paul_membre_ag.with_assets_declaration,
