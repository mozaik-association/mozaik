...
class test_coordinate_wizard(object):
    ...
    def change_main_coordinate(self, invalidate):
        ...
        wiz_id = self.model_coordinate_wizard.create(
            self.cr, self.uid, wiz_vals, context=context)
        return self.model_coordinate_wizard.button_change_main_coordinate(
            self.cr, self.uid, [wiz_id], context=context)
    ...
    def switch_main_coordinate(self, new_main_coordinate, invalidate):
        ...
        wiz_vals = {
            self.model_coordinate._discriminant_field:
            new_main_coordinate[
                self.model_coordinate._discriminant_field].id or
            new_main_coordinate[self.model_coordinate._discriminant_field],
            'invalidate_previous_coordinate': invalidate,
        }
        wiz_id = self.model_coordinate_wizard.create(
            self.cr, self.uid, wiz_vals, context=context)
    ...
    def test_mass_replication(self):
        ...
        model_coo = self.model_partner.read(
            self.cr, self.uid,
            [self.partner_id_1,
             self.partner_id_2,
             self.partner_id_3], [self.coo_into_partner], context={})
        for model_coordinate_vals in model_coo:
            coordinate = self.model_coordinate.browse(
                self.cr, self.uid,
                model_coordinate_vals[self.coo_into_partner][0], context={})
            coord_value = self.model_coordinate._is_discriminant_m2o() and \
                coordinate[self.model_coordinate._discriminant_field].id or \
                coordinate[self.model_coordinate._discriminant_field]
            self.assertEqual(
                coord_value == self.model_id_1 and coordinate.is_main,
                True,
                'Coordinate Should Be Replicate Into The Associated Partner')
    ...
    def test_mass_replication_with_invalidate(self):
        ...
        **Note**
        ...
        """
        self.change_main_coordinate(True)
        active = self.model_coordinate.read(
            self.cr, self.uid,
    ...
    def test_mass_replication_without_invalidate(self):
        ...
        **Note**
        ...
        active = self.model_coordinate.read(
            ...
            context={})['active']
        self.assertEqual(
            active, True, 'Previous model Coordinate should not be invalidate')
