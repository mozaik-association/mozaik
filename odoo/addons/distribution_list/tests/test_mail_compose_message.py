...
class TestMailComposeMessage(TransactionCase):
    ...
    def test_send_mail_with_distribution_list(self):
        ...
        mail_composer = self.mail_composer_obj.with_context(
            context).create(mail_composer_vals)
        ...
        self.assertEquals(
            len(mails), 1,
            "Should have only one mail generated by this mass mailing")
        mail = first(mails)
        self.assertEqual(
            partner.id, mail.res_id,
